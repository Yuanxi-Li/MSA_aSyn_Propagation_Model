function outputs_all = stdNDM_mouse_aSyn_Project_MSA_PermutedConnectome_Node(varargin)
% STDNDM_MOUSE_ASYN_PROJECT_MSA_PERMUTEDCONNECTOME_NODE
% Fit the MSA-alpha-Syn model using node-label permuted connectomes.
%
% In each permutation, the same node permutation is applied to both rows
% and columns of the connectome. This preserves all connectome weights and
% the complete network topology, while randomizing the correspondence
% between network nodes and the regional pathology and seeding data.
%
% Parameters fitted:
...
n_nodes = size(C_original, 1);

if size(C_original, 2) ~= n_nodes

    error('Connectome.raw must be a square matrix.');
...
%% Perform node-label connectome permutations
...
for PermutationNum = 1:1000
    ...
    try
        ...
        fprintf( ...
            'Node-label connectome permutation %d/1000\n', ...
            PermutationNum);


        %% Generate a reproducible node permutation

        rng(PermutationNum);
        rng_state = rng;

        node_permutation = randperm(n_nodes);
        ...
        % Directionality starts from s = 0.5 because node labels have been
        % randomized, while s remains freely optimized over [0,1].
        ...
        % The node permutation is small enough to store directly and allows
        % exact reconstruction of each randomized connectome.
        outputs.ndm.Full.init.node_permutation = ...
            node_permutation;
