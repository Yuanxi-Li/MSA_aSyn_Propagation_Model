function outputs_all = ...
    stdNDM_mouse_aSyn_Project_PFF_Directionality_Tests(varargin)
% STDNDM_MOUSE_ASYN_PROJECT_PFF_DIRECTIONALITY_TESTS
% Evaluate PFF model performance across fixed directionality values from
% fully anterograde to fully retrograde spreading.
%
...
%
% PFF pathology is jointly fitted at 3 and 6 MPI.
...
for directionality_n = 1:length(directionality)
    ...
    if ~logical(ipR.bootstrapping)
        ...
        fprintf( ...
            'Fitting PFF model at fixed s = %.2f\n', ...
            fixed_s);


        %% Select PFF pathology data

        % Jointly fit the 3 and 6 MPI pathology maps.
        ...
        % Use the PFF-specific nonlinear constraint across both 3 and
        % 6 MPI predictions.
