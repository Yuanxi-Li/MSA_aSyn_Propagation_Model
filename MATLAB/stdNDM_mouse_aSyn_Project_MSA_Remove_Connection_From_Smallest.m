function outputs_all = ...
    stdNDM_mouse_aSyn_Project_MSA_Remove_Connection_From_Smallest(varargin)
...
% the weakest connectome weights.
%
% Connectome_Ratio indicates the percentage of connectome matrix elements
% retained. For each ratio, the smallest 1-ratio fraction of matrix
% elements is set to zero before fitting the model.
%
...
%% Progressively remove the weakest connectome weights

tic

Connectome_Ratio = [2, 5, 10:5:95];
...
for Connectome_Num = 1:length(Connectome_Ratio)
    ...
    try
        ...
        % Remove the smallest 1-retained_ratio fraction of all connectome
        % matrix elements.
        RemoveNum = floor( ...
        ...
        if ~logical(ipR.bootstrapping)
            ...
            % Initialize at the full-connectome MSA-alpha-Syn result,
            % s = 0, while allowing free optimization over [0,1].
