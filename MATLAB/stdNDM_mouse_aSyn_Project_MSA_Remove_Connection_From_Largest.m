function outputs_all = ...
    stdNDM_mouse_aSyn_Project_MSA_Remove_Connection_From_Largest(varargin)
% STDNDM_MOUSE_ASYN_PROJECT_MSA_REMOVE_CONNECTION_FROM_LARGEST
% Evaluate MSA-alpha-Syn model performance after progressively removing
% the strongest connectome weights.
%
% Connectome_Ratio indicates the percentage of connectome matrix elements
% retained. For example, a ratio of 80 removes the largest 20% of matrix
% elements, whereas a ratio of 99 removes the largest 1%.
%
...
% The MSA-alpha-Syn seeding-scale upper bound reflects the substantially
% lower injected amount relative to the PFF condition.
...
%% Progressively remove the strongest connectome weights

tic

Connectome_Ratio = 80:1:99;

for Connectome_Num = 1:length(Connectome_Ratio)

    try

        retained_ratio = Connectome_Ratio(Connectome_Num);


        %% Construct the partial connectome

        C = Connectome.raw;

        % Remove the largest 1-retained_ratio fraction of all connectome
        % matrix elements.
        RemoveNum = floor( ...
            numel(C) * (1 - retained_ratio / 100));
        ...
        if ~logical(ipR.bootstrapping)
            ...
            fprintf( ...
                'Fitting MSA-alpha-Syn model after retaining %g%% ', ...
                retained_ratio);

            fprintf('of connectome matrix elements\n');


            %% Select MSA-alpha-Syn pathology data

            % Model only the early 0-3 MPI phase.
            ...
            %
            % The partial-connectome models are initialized at the
            % full-connectome MSA-alpha-Syn result, s = 0, while s remains
            % freely optimized over [0,1].
            ...
            % The partial connectome is not stored to avoid duplicating a
            % full matrix for every retained-ratio result.
            ...
            outputs.ndm.Full.results.LinR_Mean = ...
                mean(LinRvalues);

            outputs.ndm.Full.results.New_R_Log = NewRLog;

            outputs.ndm.Full.results.LogLinR = ...
                LogLinRvalues;
            ...
            if logical(ipR.verbose)
                ...
                disp(['Connectome ratio = ' ...
                    num2str(retained_ratio)])
            ...
            error([ ...
                ...
                'partial-connectome analysis. Set bootstrapping to 0.']);

        end


        %% Store the current connectome-ratio result
