function outputs_all = stdNDM_mouse_aSyn_Project_PFF_Gene(varargin)
% STDNDM_MOUSE_ASYN_PROJECT_PFF_GENE
% Fit the synaptic-locus-specific gene-modulated eNDM to PFF data.
%
...
%
% PFF pathology is jointly modeled at 3 and 6 MPI. The parameter
% initialization and bounds below are preserved from the supplied code.
...
% Preserve the original PFF parameter bounds.
ub_ = [1, Inf, Inf, 1, 1];
lb_ = zeros(1, 5);
lb_(2) = -Inf;
...
% Preserve the original PFF optimization settings.
opttol_ = 1e-6;
fxntol_ = 1e-6;
steptol_ = 1e-12;
maxeval_ = 100000;
...
for gene_num = 1:length(gene_names_trans)
    try
        ...
        if ~logical(ipR.bootstrapping)
            ...
            fprintf( ...
                'Fitting PFF gene %d/%d\n', ...
                gene_num, length(gene_names_trans));

            time_stamps = tpts.PFF_Average;

            % Reconstruct the two PFF group-median pathology maps from the
            % current input structure.
            pathology = [median(Pathology_Data.PFF_Individual_3_MPI, 2), ...
                median(Pathology_Data.PFF_Individual_6_MPI, 2)];
            ...
            %% Define PFF directionality initialization and bounds

            % Preserve the supplied PFF setting exactly: fix s at
            % 0.844337160484113. Lambda  retains its
            % original [0,1] bounds.
            if ~logical(ipR.w_dir)
                ipR.param_init(4) = 0.844337160484113;
                ipR.ub(4) = 0.844337160484113;
                ipR.lb(4) = 0.844337160484113;
            end
            ...
            [param_num, fval_num] = fmincon( ...
                ...
                'objfun_eNDM_general_dir_nlcon_gene_PFF', options);
