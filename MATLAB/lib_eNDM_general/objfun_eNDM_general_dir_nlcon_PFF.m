function [c, ceq] = objfun_eNDM_general_dir_nlcon_PFF(param)
% OBJFUN_ENDM_GENERAL_DIR_NLCON_PFF
% Define nonlinear constraints for fitting the directional eNDM to PFF.
%
% The constraint requires all predicted pathology values at both
% PFF time points to remain no greater than 1.
%
% Parameter order:
...
%% Define PFF time points

% PFF is jointly modeled using pathology at 3 and 6 MPI.
ts = tpts.PFF_Average;
...
%
% Because PFF contains two modeled time points, y has dimensions
% n_ROI x 2. Vectorization applies the upper-bound constraint separately
% to every region at both time points.
c = y(:) - 1;
