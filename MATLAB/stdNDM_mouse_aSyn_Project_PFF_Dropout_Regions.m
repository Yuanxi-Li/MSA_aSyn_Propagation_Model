function outputs_all = stdNDM_mouse_aSyn_Project_PFF_Dropout_Regions(varargin)
% STDNDM_MOUSE_ASYN_PROJECT_PFF_DROPOUT_REGIONS
% Assess the robustness of PFF model fitting using measurement-level
% dropout subsampling.
%
% For each iteration, 50% of the available mouse-level regional pathology
% measurements are randomly removed. Median regional pathology maps are
% then reconstructed separately at 3 and 6 MPI, and the directional eNDM
% is jointly refitted to both time points.
%
...
% Regional pathology is log-transformed after the dropout median maps are
% reconstructed.
normtype_ = 'log';
...
for BootstrapNum = 1:1000
    ...
    try
        ...
        if ~logical(ipR.bootstrapping)
            ...
            % PFF pathology is jointly modeled at 3 and 6 MPI.
            time_stamps = tpts.PFF_Average;

            % The 3 MPI group contains four mice, and the 6 MPI group
            % contains three mice. The measurements are concatenated here
            % before measurement-level dropout.
            pathology_orig = [ ...
                Pathology_Data.PFF_Individual_3_MPI, ...
                Pathology_Data.PFF_Individual_6_MPI];
            ...
            % measurements from both time points. Missing measurements
            % already present in the original data are not sampled.
            ...
            %% Reconstruct the regional pathology maps

            % Reconstruct the median regional map separately for the four
            % mice at 3 MPI and the three mice at 6 MPI.
            pathology_temp_median = [ ...
                median(pathology_temp(:, 1:4), 2, 'omitnan'), ...
                median(pathology_temp(:, 5:7), 2, 'omitnan')];

            % Preserve the log transformation used in the original
            % measurement-dropout analysis.
            pathology = normalizer(pathology_temp_median, 'log');
            ...
            %
            % The full-data PFF model yielded an optimal directionality of
            % approximately s = 0.84. The dropout fits are therefore
            % initialized at s = 0.84, while directionality remains freely
            % optimized over the complete interval [0,1].

            if ~logical(ipR.w_dir)

                ipR.param_init(4) = 0.84;
                ipR.ub(4) = 1;
                ipR.lb(4) = 0;
            ...
            % The PFF-specific nonlinear constraint requires all predicted
            % pathology values at both fitted time points to remain less
            % than or equal to 1.
